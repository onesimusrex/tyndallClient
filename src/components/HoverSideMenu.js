            ...
            <div className="list-group panel">
              ...
              {this.props.sideMenu != null && sideMenu.map((level1, index1) =>
                ...
                <a csi={level1.code} onClick={this.upLevelClick} href={"#menu"+(index1+1)} className="list-group-item collapsed sideLink" data-toggle="collapse" data-parent="#sidebar" aria-expanded="false">
                  ...
                  <span className="hidden-sm-down">
                    {level1.title.length < 30 ? " | " + level1.title : " | " + level1.title.slice(0,30).trim()+"..."}
                  </span>
                </a>
                ...
                {level1.children.map((level2, index2) =>
                  <div className="collapse" id={"menu"+(index1+1)}>
                      ...
                      {/* className="text-white-50" */}
                        ...
                        <span >
                        {level2.title.length < 30 ? " | " + level2.title : " | " + level2.title.slice(0,30).trim()+"..."}
                        </span>
                      </a>
                      {/* {console.log(level2)} */}
                      {level2.children.map((level3, index3) => 
                        <div className="collapse" id={"menu"+(index1+1)+"sub"+(index2+1)}>
                          <a csi={level3.code} onClick={this.upLevelClick} href={"#menu"+(index1+1)+"sub"+(index2+1)+"sub"+(index3+1)} className="list-group-item sideLink" data-toggle="collapse" aria-expanded="false">
                              ...
                              <span >
                                {level3.title.length < 30 ? " | " + level3.title : " | " + level3.title.slice(0,30).trim()+"..."}
                              </span>
                          ...
                          // onClick={this.LeafClick.bind(this)}
                            ...
                            <a onClick={this.LeafClick.bind(this)} id={level4.code} csi={level4.code} href="javascript:void(0)" className="list-group-item sideLink sideResult" data-parent={"#menu"+(index1+1)+"sub"+(index2+1)+"sub"+(index3+1)}>
                              ...
                              <span onClick={this.LeafClick.bind(this)} id={level4.code}>
                                {level4.title.length < 30 ? " | " + level4.title : " | " + level4.title.slice(0,30).trim()+"..."}
                              </span>
                            </a>
                        </div>
